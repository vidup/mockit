...
describe("Change function behaviour combined", () => {
    const mock = Mockit.mock2(Person);
    Mockit.when2(mock).calls("walk").withArgs(1, 2, 3).thenReturn(42);
    Mockit.when2(mock).calls("walk").withArgs(4, 5, 6).thenResolve(33);
    Mockit.when2(mock).calls("walk").withArgs(7, 8, 9).thenThrow("error");
    Mockit.when2(mock).calls("walk").withArgs(10, 11, 12).thenReject("REJECTED");
    Mockit.when2(mock).calls("walk").withArgs(13, 14, 15).thenCall(() => {});
